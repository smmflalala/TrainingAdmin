document.write("<script language=javascript src='../js/request.js'></script>");
    ...
    tableSelect.prototype.render = function (opt) {
        ...
        elem.off('click').on('click', function(e) {
            ...
            form.on('submit(tableSelect_btn_search)', function(data){
				var search_key = data.field.teacherName
				console.log(search_key)
					console.log(data)
				if(search_key){
					console.log(data)
					var search_info = {
						api: 'teacher/name/' + search_key,
						type: "GET",
						data: []
					}
					
					function search_info_func(e){
						if(e.success) {
							...
							opt.table.elem = "#"+tableName;
							opt.table.id = tableName;
							opt.table.data = teacher_data_fiexd;
							table.render(opt.table);
					...
					request_(search_info, search_info_func)
					
				}else{
					// console.log('空')
					opt.table.url = BASE_URL + "teacher/1/0";
					table.render(opt.table);
				}
